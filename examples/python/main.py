"""
Example: How to call the apify-run-queue actor using Apify API Client

For full documentation of the Apify Client for Python, see: https://docs.apify.com/api/client/python
For full documentation of the actor, see: https://apify.com/lexis-solutions/apify-run-queue
...
def run_actor():
    """Run the apify-run-queue actor and retrieve results."""
    ...
    actor_id = "lexis-solutions/apify-run-queue"
    print(f"Starting actor: {actor_id}")

    run = client.actor(actor_id).call(
        run_input={
            "actorCalls": [{"actorId":"apify/website-content-crawler","input":{"startUrls":[{"url":"https://www.apify.com"}]}}],
        }
    )
